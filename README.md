...
金字塔中的每张图片表示了一个区域（Area），可理解为生物群系在生成时的一个中间状态。区域经过从金字塔底端到顶端的下采样（Down Sample），以及从金字塔顶端到底端的上采样（Up Sample）之后，就完成了其内的生物群系生成。
...
Zoom层还有个孪生兄弟就是生成链开头的VoronoiZoom层。与Zoom的放大2倍不同，VoronoiZoom给区域放大4倍。
...
别急着开香槟，尽管三线性插值已经保证了一定程度上的地形过渡，但是群系交界处的高度过渡仍然不够平滑。你也不想看到在短短4格内就从海平面上升至云层的迫真高原地形吧。
...
那么，引入3维噪声之后，群系的高度变化率起作用了吗？稍安勿躁，我们以区元顶点高度为横轴，密度为纵轴绘图，可以得到一个一次函数图像（实线部分），它显示了不同高度下的基础密度。
...
Simplex噪声其实就是Perlin噪声的三角晶格变体，它与Perlin噪声略有不同：
...
世界中随处可见的草、树木、矿石等等，大多属于群系装饰。由于特征种类繁多，这里不展开细讲，有兴趣可以借助本人提供的FeatureHelper模组进行研究。
